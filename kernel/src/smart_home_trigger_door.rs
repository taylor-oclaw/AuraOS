...
impl SmartHomeTriggerDoor {
    ...
    pub fn open(&mut self) {
        if !self.is_open {
            ...
            println!("{} has been opened.", self.name);
        } else {
            println!("{} is already open.", self.name);
        }
    }
    ...
    pub fn close(&mut self) {
        if self.is_open {
            ...
            println!("{} has been closed.", self.name);
        } else {
            println!("{} is already closed.", self.name);
        }
    }

    pub fn add_trigger(&mut self, trigger: &str) {
        self.triggers.push(String::from(trigger));
        println!("Trigger '{}' added to {}.", trigger, self.name);
    }
    ...
    pub fn remove_trigger(&mut self, trigger: &str) -> bool {
        if let Some(index) = self.triggers.iter().position(|t| t == trigger) {
            ...
            println!("Trigger '{}' removed from {}.", trigger, self.name);
            true
        } else {
            println!("Trigger '{}' not found in {}.", trigger, self.name);
            false
        }
    }
