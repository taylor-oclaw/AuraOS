...
impl JapaneseIME {
    ...
    pub fn generate_candidates(&mut self) {
        ...
        if !self.conversion_buffer.is_empty() {
            self.candidates.push(format!("Candidate 1: {}", self.conversion_buffer));
            self.candidates.push(format!("Candidate 2: {}", self.conversion_buffer));
            self.candidates.push(format!("Candidate 3: {}", self.conversion_buffer));
        }
    }
