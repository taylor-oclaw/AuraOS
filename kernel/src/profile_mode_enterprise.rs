...
impl ProfileModeEnterprise {
    ...
    pub fn set_user(&mut self, user: &str) {
        ...
        self.log(format!("User set to {}", self.user).as_str());
    }
    ...
    pub fn add_permission(&mut self, permission: &str) {
        if !self.permissions.contains(&String::from(permission)) {
            ...
            self.log(format!("Permission added: {}", permission).as_str());
        } else {
            self.log(format!("Permission already exists: {}", permission).as_str());
        }
    }
    ...
    pub fn remove_permission(&mut self, permission: &str) {
        if let Some(index) = self.permissions.iter().position(|p| p == permission) {
            ...
            self.log(format!("Permission removed: {}", permission).as_str());
        } else {
            self.log(format!("Permission not found: {}", permission).as_str());
        }
    }

    pub fn log_status(&self) {
        self.log("Current profile status:");
        self.log(format!("User: {}", self.user).as_str());
