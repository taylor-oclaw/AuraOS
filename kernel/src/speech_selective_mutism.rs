...
impl SpeechSelectiveMutism {
    ...
    pub fn activate(&mut self) {
        self.is_active = true;
        self.log.push(format!("User {} activated speech selective mutism.", self.user_id));
    }

    pub fn deactivate(&mut self) {
        self.is_active = false;
        self.log.push(format!("User {} deactivated speech selective mutism.", self.user_id));
    }

    pub fn add_allowed_phrase(&mut self, phrase: String) {
        self.allowed_phrases.push(phrase);
        self.log.push(format!("Added allowed phrase for user {}: {}", self.user_id, &self.allowed_phrases.last().unwrap()));
    }

    pub fn add_blocked_phrase(&mut self, phrase: String) {
        self.blocked_phrases.push(phrase);
        self.log.push(format!("Added blocked phrase for user {}: {}", self.user_id, &self.blocked_phrases.last().unwrap()));
    }
