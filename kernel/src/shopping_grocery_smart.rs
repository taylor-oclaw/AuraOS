...
pub extern "C" fn rust_start() -> ! {
    ...
    println!("Total items: {}", grocery_list.total_items());
    println!("Total quantity: {}", grocery_list.total_quantity());

    if let Some(item) = grocery_list.find_item("Milk") {
        println!("Found Milk with quantity: {}", item.quantity);
    } else {
        println!("Milk not found");
    }

    grocery_list.remove_item(String::from("Bread"));
    println!("After removing Bread, total items: {}", grocery_list.total_items());
