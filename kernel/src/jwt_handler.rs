...
impl JwtHandler {
    ...
    pub fn encode(&self, payload: &str) -> Option<String> {
        ...
        let signature = self.sign(&format!("{}.{}", encoded_header, encoded_payload));
        Some(format!("{}.{}.{}", encoded_header, encoded_payload, signature))
    }
    ...
    pub fn decode(&self, token: &str) -> Option<String> {
        ...
        let signature = self.sign(&format!("{}.{}", parts[0], parts[1]));
