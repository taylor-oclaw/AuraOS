...
impl SmartHomeEnergyMonitor {
    ...
    pub fn add_device(&mut self, device_name: &str) {
        if self.is_active {
            ...
            println!("Monitor is not active. Cannot add devices.");
        }
    }
    ...
    pub fn update_power_usage(&mut self, power_usage: u32) {
        if self.is_active {
            ...
            println!("Current power usage updated to {} watts.", power_usage);
        } else {
            println!("Monitor is not active. Cannot update power usage.");
        }
    }
