...
impl FlightTrack {
    ...
    pub fn get_flight_info(&self) -> String {
        let mut info = format!("Flight ID: {}\n", self.flight_id);
        info.push_str(format!("Destination: {}\n", self.destination).as_str());
        info.push_str(format!("Passengers: {:?}\n", self.passengers).as_str());
        info.push_str(format!("Altitude: {} meters\n", self.altitude).as_str());
        info.push_str(format!("Speed: {} km/h\n", self.speed).as_str());
        info
    }
}
