...
impl VulkanRenderer {
    ...
    pub fn create_window(&mut self, title: &str, width: u32, height: u32) {
        ...
        self.width = width;
        self.height = height;
        println!("Creating window with title: {} and dimensions: {}x{}", title, width, height);
    }

    pub fn load_shader(&mut self, path: &str) {
        self.shaders.push(String::from(path));
        println!("Shader loaded from: {}", path);
    }

    pub fn render_frame(&self) {
        // Render a frame using the loaded shaders
        println!("Rendering frame with {} shaders", self.shaders.len());
    }
}
