...
impl MarketplaceOrgVolumeLicense {
    ...
    pub fn get_license_info(&self) -> Vec<String> {
        vec![
            format!("Product Name: {}", self.product_name),
            format!("License Key: {}", self.license_key),
            format!("Volume Count: {}", self.volume_count),
            format!("Expiration Date: {}", self.expiration_date),
            format!("Is Active: {}", self.is_active),
        ]
    }
}
