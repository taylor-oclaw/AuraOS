...
impl SkillRetryHandler {
    ...
    fn schedule_retry(&self) {
        ...
        println!("Scheduling retry for skill: {} in {} ms", self.skill_name, self.retry_interval);
    }
}
...
fn main() {
    ...
    handler.set_success_callback(|| {
        println!("Skill execution succeeded!");
    });

    handler.set_failure_callback(|| {
        println!("Skill execution failed after max retries.");
    });
