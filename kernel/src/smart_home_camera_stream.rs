...
impl SmartHomeCameraStream {
    ...
    pub fn start_recording(&mut self) {
        if !self.is_recording {
            ...
            println!("Recording started for stream ID {}", self.stream_id);
        }
    }
    ...
    pub fn stop_recording(&mut self) {
        if self.is_recording {
            ...
            println!("Recording stopped for stream ID {}", self.stream_id);
        }
    }
    ...
    pub fn set_resolution(&mut self, resolution: (u32, u32)) {
        ...
        println!("Resolution set to {:?}", self.resolution);
    }
    ...
    pub fn capture_frame(&mut self, frame_data: &[u8]) {
        if self.is_recording {
            ...
            println!("Frame captured for stream ID {}", self.stream_id);
        }
    }
    ...
    pub fn clear_video_buffer(&mut self) {
        ...
        println!("Video buffer cleared for stream ID {}", self.stream_id);
    }
}
